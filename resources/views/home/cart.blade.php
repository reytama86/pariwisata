                    ...
                    <div class="col-md-12">
                        <div class="table-wrap mb-30">
                            <table class="shop_table cart table">
                                <thead>
                                    <tr>
                                        <th class="package-name" colspan="2">package</th>
                                        <th class="package-price">Price</th>
                                        <th class="package-quantity">Quantity</th>
                                        <th class="package-subtotal" colspan="2">Total</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($carts as $cart)
                                        <input type="hidden" name="id_package[]" value="{{ $cart->package->id }}">
                                        <input type="hidden" name="jumlah[]" value="{{ $cart->jumlah }}">
                                        <input type="hidden" name="total[]" value="{{ $cart->total }}">
                                        <tr class="cart_item">
                                            <td class="package-thumbnail">
                                                <a href="#">
                                                    <img src="/uploads/{{ $cart->package->gambar }}" alt="">
                                                </a>
                                            </td>
                                            <td class="package-name">
                                                <a href="#">{{ $cart->package->nama_package }}</a>
                                                <ul>
                                                    <li>{{ $cart->size }}</li>
                                                    <li>{{ $cart->color }}</li>
                                                </ul>
